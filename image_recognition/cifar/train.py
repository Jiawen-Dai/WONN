...
from image_recognition.cifar.data import build_dataloaders, CIFAR_STATS
...
def evaluate(net, dataloader, device, is_main=True, is_ddp=False, log_fh=None):
    ...
    log_print(f"Accuracy of the network on the test images: {acc:.2f}%", log_fh, is_main)
...
def build_parser():
    parser = argparse.ArgumentParser(description="WONN CIFAR Training")
...
def main():
    ...
    num_classes = CIFAR_STATS[args.data]["num_classes"]
    ...
    log_print(
        f"Dataset={args.data} | train={len(trainloader.dataset)} | "
        f"test={len(testloader.dataset)} | num_classes={num_classes}",
        log_fh,
        is_main,
    )
    ...
    ema = EMA(base_net, beta=args.beta, update_every=10, update_after_step=200)
    ...
    scheduler = optim.lr_scheduler.ConstantLR(optimizer, factor=1.0, total_iters=args.epochs)
    ...
    try:
        for epoch in range(args.epochs):
            ...
            if args.eval_freq > 0 and ((epoch + 1) % args.eval_freq) == 0:
                ...
                if is_main and writer is not None:
                    writer.add_scalar("model/test accuracy", acc, epoch)

                if is_main and writer_ema is not None:
                    writer_ema.add_scalar("ema/test accuracy", ema_acc, epoch)

            if ((epoch + 1) % args.checkpoint_every) == 0:
                ddp_barrier(is_ddp)

                if is_main:
                    save_checkpoint(base_net, optimizer, epoch, loss, checkpoint_dir=jobdir)
